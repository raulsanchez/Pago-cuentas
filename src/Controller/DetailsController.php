...
class DetailsController extends AppController
{
    ...
    public function month($month = null)
    {
        ...
        foreach ($products as $product):
            if ($product->pro_payment == 0):
                array_push($view_products, $product); else:
        ...
        if (date('Y') < $year_end):
                    array_push($view_products, $product); elseif (date('Y') == $year_end):
    ...
    public function payment($date_buy, $date_now)
    {
        ...
        if ($second_diff < 0):
            return 999999; elseif ($second_diff > 29203200):
            $anio = $second_diff / 29203200;

        return intval(date('m', $second_diff) + (12 * intval($anio))); else:
